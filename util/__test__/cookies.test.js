...
test('addProductByProductId returns correct value of 2 on two calls', () => {
  const testId = 1;
  const testQuantity1 = [{ id: testId, quantity: 2 }];
  const testQuantity2 = [{ id: testId, quantity: 3 }];
  const result1 = addProductByProductId(testId);
  const result2 = addProductByProductId(testId);

  expect(result1).toEqual(testQuantity1);
  expect(result2).toEqual(testQuantity2);
});

//  2. Test: Remove product in shopping cart cookies
...
// 3. Test: Decrease product quantity in shopping cart cookies
